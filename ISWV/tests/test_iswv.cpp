...
TEST_CASE("CiA 402 transition accepts switch-on-disabled status with bit 5 set")
{
    ...
    transport->set_send_hook([transport, &status](const iswv::CanFrame& request) {
    ...
    CHECK(drive.transition_to(iswv::cia402::DriveState::switch_on_disabled,
...
TEST_CASE("CiA 402 enable reports fault status with bit 5 set before issuing commands")
{
    ...
    transport->set_send_hook([transport, &control_writes](const iswv::CanFrame& request) {
        const auto object = iswv::test::request_object(request);
        if (request.data[0] == 0x40 && object == iswv::objects::status_word.address) {
    ...
    auto result = drive.enable({std::chrono::milliseconds{300},
                                std::chrono::milliseconds{1},
                                {std::chrono::milliseconds{100}}});
    CHECK(!result);
...
TEST_CASE("CiA 402 enable follows 6, 7, F state sequence")
{
    ...
    transport->set_send_hook([transport, &mutex, &status](const iswv::CanFrame& request) {
        const auto object = iswv::test::request_object(request);
        if (request.data[0] == 0x40 && object == iswv::objects::status_word.address) {
    ...
    CHECK(drive.enable({std::chrono::milliseconds{300},
                        std::chrono::milliseconds{1},
                        {std::chrono::milliseconds{100}}}));
...
TEST_CASE("CAN0 steering layout maps four steering motors by wheel position")
{
    ...
    CHECK_EQ(steering.at(iswv::WheelPosition::front_left)->configuration().kind,
             iswv::AxisKind::steering);
    CHECK_EQ(steering.at(iswv::WheelPosition::rear_left)->configuration().kind,
             iswv::AxisKind::steering);
    CHECK_EQ(steering.at(iswv::WheelPosition::rear_right)->configuration().kind,
             iswv::AxisKind::steering);
    CHECK_EQ(steering.at(iswv::WheelPosition::front_right)->configuration().kind,
             iswv::AxisKind::steering);
}
...
TEST_CASE("SafetyPolicy issues quick stop after supervision timeout")
{
    ...
    auto throwing_state_observer = axis.on_state([](const iswv::AxisState&) {
        throw std::runtime_error("observer failure");
    });

    std::promise<std::uint16_t> command_promise;
    auto command_future = command_promise.get_future();
    transport->set_send_hook([&command_promise](const iswv::CanFrame& request) {
