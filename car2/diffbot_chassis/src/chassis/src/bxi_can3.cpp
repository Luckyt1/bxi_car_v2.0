...
void BxiCan3::install_handlers()
{
  // 接收线程可能已复制回调；捕获共享状态，避免析构后访问 BxiCan3 本体。
...
void BxiCan3::save_zero_positions()
{
  throw_if_fault_latched();
  if (!initialized_ ||
...
void BxiCan3::hold_zero(std::chrono::steady_clock::time_point now)
{
  throw_if_fault_latched();
...
void BxiCan3::enable(std::uint32_t id)
{
...
void BxiCan3::command(std::uint32_t id, const bxi::Command & command)
{
  const auto index = motor_index(id);
...
std::string BxiCan3::take_feedback_diagnostic(std::chrono::steady_clock::time_point now)
{
  std::lock_guard<std::mutex> lock(feedback_state_->mutex);
...
void BxiCan3::set_target_degrees(std::uint32_t id, double degrees)
{
  const auto index = motor_index(id);
...
double BxiCan3::target_degrees(std::uint32_t id) const
{
  const auto index = motor_index(id);
  std::lock_guard<std::mutex> lock(feedback_state_->mutex);
...
std::string BxiCan3::fault_reason() const
{
  std::lock_guard<std::mutex> lock(feedback_state_->mutex);
...
void BxiCan3::stop_noexcept() noexcept
{
  try {
    stop();
  } catch (...) {
...
void BxiCan3::throw_if_fault_latched() const
{
...
void BxiCan3::latch_feedback_loss_if_needed(std::chrono::steady_clock::time_point now)
{
  {
    std::lock_guard<std::mutex> lock(feedback_state_->mutex);
...
void BxiCan3::latch_fault_locked(
