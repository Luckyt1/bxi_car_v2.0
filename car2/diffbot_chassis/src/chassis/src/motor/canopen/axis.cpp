...
public:
  Impl(
    std::shared_ptr<canopen::CanopenNode> node_in,
...
Result<void> Axis::configure_default_pdos(
...
Result<void> Axis::configure_velocity_mode(
  bool immediate,
...
Result<void> Axis::command_travel_velocity_mps(
    ...
    1, [raw = raw.value(), mode](canopen::ObjectAddress object) {
      if (object == objects::control_word.address) {
        return Result<canopen::MappedValue>::success(
          canopen::mapped_value(
            objects::control_word,
...
Result<void> Axis::configure_position_mode(
  double profile_velocity_rpm,
  double acceleration_rps2,
  double deceleration_rps2,
  canopen::SdoOptions sdo)
{
  ...
  result = impl_->node->write(
    objects::profile_velocity,
...
Result<void> Axis::move_continuous_absolute_inc(
  std::int32_t target,
  canopen::SdoOptions sdo)
{
  auto result = impl_->node->write(objects::target_position, target, sdo);
...
Result<void> Axis::configure_homing(
  const HomingConfiguration & configuration,
  canopen::SdoOptions sdo)
{
  ...
  result = impl_->node->write(
    objects::homing_switch_velocity,
    static_cast<std::uint32_t>(switch_velocity.value()), sdo);
  if (!result) {
    return result;
  }
  result = impl_->node->write(
    objects::homing_zero_velocity,
  ...
  canopen::SdoOptions sdo)
{
  ...
  return impl_->node->write(
    objects::ecan_sync_clock_mode,
...
Result<void> Axis::set_software_limits(
...
Result<void> Axis::set_stop_options(
  std::int16_t quick_stop,
...
Result<void> Axis::set_can_parameters(
  std::uint16_t new_node_id,
