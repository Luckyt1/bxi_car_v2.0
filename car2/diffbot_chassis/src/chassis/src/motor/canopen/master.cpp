...
public:
  ...
  void start()
  {
    try {
      ...
      transport_->set_receive_handler(
        [weak](const CanFrame & frame) {
          if (auto self = weak.lock()) {
            self->receive(frame);
          }
        });
      transport_->set_error_handler(
        [weak](const Error & error) {
          if (auto self = weak.lock()) {
    ...
    std::function<void(Result<SdoResponse>)> completion)
  {
    if (options.timeout.count() <= 0) {
      enqueue_event(
        true, [completion = std::move(completion)]() mutable {
          completion(
            Result<SdoResponse>::failure(
  ...
  {
    return add_node_callback(
      heartbeat_callbacks_, node_id, 0, std::move(callback),
  ...
  {
    return add_node_callback(
      timeout_callbacks_, node_id, 0, std::move(callback),
  ...
  {
    return add_node_callback(
      emergency_callbacks_, node_id, 0, std::move(callback),
      CallbackType::emergency);
  }

  Subscription add_pdo_callback(
    std::uint8_t node_id,
    std::uint8_t number,
    std::function<void(const PdoEvent &)> callback)
  {
    return add_node_callback(
      pdo_callbacks_, node_id, number, std::move(callback),
      CallbackType::pdo);
  }
...
private:
  ...
  void transport_error(Error error)
  {
    ...
    enqueue_event(
      false, [error = std::move(error), callbacks = std::move(callbacks)] {
  ...
  void protocol_loop()
  {
    ...
    while (!stopping_.load()) {
      std::deque<CanFrame> frames;
      {
        ...
        receive_cv_.wait_for(
          lock, options_.timer_resolution, [this] {
  ...
  void process_heartbeat(const CanFrame & frame, bool forced_guard)
  {
    ...
    enqueue_event(
      false, [event, callbacks = std::move(callbacks)] {
  ...
  void emit_raw(const CanFrame & frame)
  {
    ...
    if (!callbacks.empty()) {
      enqueue_event(
        false, [frame, callbacks = std::move(callbacks)] {
  ...
  void complete_sdo(
    const std::shared_ptr<SdoTransaction> & transaction,
...
void CanopenNode::download_async(
  ObjectAddress object,
  const std::uint8_t * data,
  std::size_t size,
  SdoOptions options,
...
std::future<Result<SdoResponse>> CanopenNode::upload(
  ObjectAddress object,
  SdoOptions options)
{
  ...
  upload_async(
    object, options, [promise](Result<SdoResponse> result) mutable {
      promise->set_value(std::move(result));
    });
  return future;
}

std::future<Result<void>> CanopenNode::download(
  ObjectAddress object,
  const std::uint8_t * data,
  std::size_t size,
  SdoOptions options)
...
Result<SdoResponse> CanopenNode::upload_blocking(
  ObjectAddress object,
  SdoOptions options)
{
...
Result<void> CanopenNode::download_blocking(
  ObjectAddress object,
  const std::uint8_t * data,
  std::size_t size,
  SdoOptions options)
...
Result<void> CanopenNode::configure_pdo(
  const PdoConfiguration & input,
  SdoOptions options)
{
  ...
  auto result = write(
    Object<std::uint32_t>{{communication, 1}, "PDO COB-ID"},
  ...
  result = write(
    Object<std::uint8_t>{{mapping, 0}, "PDO map count"},
    static_cast<std::uint8_t>(configuration.mapping.size()), options);
  if (!result) {
    return result;
  }
  result = write(
    Object<std::uint8_t>{{communication, 2}, "PDO transmission type"},
  ...
  if (configuration.direction == PdoDirection::transmit) {
    result = write(
      Object<std::uint16_t>{{communication, 3}, "TPDO inhibit time"},
      configuration.inhibit_time, options);
    if (!result) {
      return result;
    }
    result = write(
      Object<std::uint16_t>{{communication, 5}, "TPDO event timer"},
...
Result<void> CanopenNode::disable_pdo(
...
Result<void> CanopenNode::send_rpdo(
  std::uint8_t number,
  const std::uint8_t * data,
  std::size_t size)
{
...
Result<void> CanopenNode::configure_heartbeat(
  ...
  SdoOptions options)
{
  ...
  if (write_producer_to_device) {
    auto written = write(
      Object<std::uint16_t>{{0x1017, 0x00},
  ...
  SdoOptions options)
{
  ...
  if (write_configuration_to_device) {
    auto written = write(
      Object<std::uint16_t>{{0x100C, 0x00}, "Guard time"},
      guard_time.count(), options);
    if (!written) {
      return written;
    }
    written = write(
      Object<std::uint8_t>{{0x100D, 0x00}, "Life-time factor"},
...
Subscription CanopenNode::on_tpdo(
  std::uint8_t number,
  std::function<void(const PdoEvent &)> callback)
{
  if (number < 1 || number > 4) {
