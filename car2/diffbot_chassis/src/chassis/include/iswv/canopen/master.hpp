...
public:
  ...
  std::future<Result<SdoResponse>> upload(
    ObjectAddress object,
    SdoOptions options = {});
  std::future<Result<void>> download(
    ObjectAddress object,
    const std::uint8_t * data,
    std::size_t size,
    SdoOptions options = {});

  Result<SdoResponse> upload_blocking(
    ObjectAddress object,
    SdoOptions options = {});
  Result<void> download_blocking(
    ObjectAddress object,
    const std::uint8_t * data,
    std::size_t size,
    SdoOptions options = {});
    ...
    DownloadCallback callback)
  {
    ...
    const auto bytes = encode_little_endian(static_cast<T>(value));
    download_async(
      object.address, bytes.data(), bytes.size(), options,
      std::move(callback));
  }

  Result<void> send_nmt(NmtCommand command);
  Result<void> configure_pdo(
    const PdoConfiguration & configuration,
    SdoOptions options = {});
  Result<void> disable_pdo(
    PdoDirection direction,
    std::uint8_t number,
    SdoOptions options = {});
  Result<void> send_rpdo(
    std::uint8_t number,
    const std::uint8_t * data,
    std::size_t size);
